...
dataset = Dataset(kind='buchwald_d')

tasks = pickle.load(open(f'{__datasets__}/dataset_buchwald/buchwald_tasks.pkl', 'rb'))
bests = pickle.load(open(f'{__datasets__}/dataset_buchwald/buchwald_best.pkl', 'rb'))
		...
		try:
			...
			if (model_kind not in META_PLANNERS) and (model_kind!='Botorch'):
				...
				while len(campaign.values) < BUDGET:
					...
					if check_convergence(samples, stop_after, bests, target_task_ix):
