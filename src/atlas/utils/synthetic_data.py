...
def trig_factory(
    ...
    for i in range(num_samples):
        ...
        amplitude = (amplitude_range[0]-amplitude_range[1])*torch.rand(1)+amplitude_range[1]
        shift = (shift_range[0]-shift_range[1])*torch.rand(1)+shift_range[1]
        ...
        if as_numpy:
            ...
            train_tasks.append(
                {'params': domain.reshape(-1, 1), 'values': values.reshape(-1, 1)}
            )
    return train_tasks
...
def gp_factory(
    ...
    k = kernel_fn(
        input_dim=param_dim,
        lengthscale=np.random.uniform(length_scale_range[0], length_scale_range[1]),
