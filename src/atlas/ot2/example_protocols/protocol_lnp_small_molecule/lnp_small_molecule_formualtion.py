...
def run(protocol: protocol_api.ProtocolContext):
    ...
    # Set up Pipettes
    p300 =      protocol.load_instrument ('p300_single_gen2', 'right',tip_racks=[p300t_1,p300t_2,p300t_3]) 
    p300_8 =    protocol.load_instrument ('p300_multi_gen2', 'left',tip_racks=[p300t_1,p300t_2,p300t_3])
    ...
    organic =           protocol.load_labware('allenlabresevoir_96_wellplate_2200ul','4')
    LNP =               protocol.load_labware('corning_96_wellplate_360ul_flat','2')
    harden_plate =      protocol.load_labware('allenlabresevoir_96_wellplate_2200ul','3')
    ...
    def matrix(pipette, stock, destination, volume, rows, columns):
        ...
        for y in range (columns):
           for x in range (rows):
               character = letters[x]
               num = numbers[y]
               if (volume[i]>0):organic_transfer(pipette,stock,volume[i],destination[character + num],max_pv)
               i=i+1
        pipette.drop_tip()
    ...
    def aqueous_phase():
        ...
        for i in range(0,12): # iterate over columns 8 rows, 12 columns
            ...
            p300_8.transfer(250,harden_stock,LNP.rows()[0][i],blow_out=True,blowout_location='destination well',new_tip='never')
        p300_8.drop_tip()
    ...
    def organic_phase():
        ...
        # cholestrol
        matrix(p300,cholestrol_stock,organic,cholestrol,8,int(formulations/8))
        ...
        # peglipid 
        matrix(p300,peglipid_stock,organic,peglipid,8,int(formulations/8))
    ...
    def formulation():
        ...
        for i in range(0,int(formulations/8)):
            for j in range (0,int(96/formulations)):
                p300_8.pick_up_tip()
                p300_8.transfer(50,organic.rows()[0][i],LNP.rows()[0][i*int(96/formulations)+j],mix_after=(30,200),new_tip='never')
                p300_8.drop_tip()
    ...
    def hardening():
        ...
        for i in range (0,12):
            p300_8.transfer(900,harden_stock,harden_plate.rows()[0][i],blow_out=True,blowout_location='destination well',new_tip='never')
        p300_8.drop_tip()
        ...
        for i in range (0,12):
            p300_8.pick_up_tip()
            p300_8.transfer(100,LNP.rows()[0][i],harden_plate.rows()[0][i],new_tip='never')
