...
from atlas.ot2.example_protocols import ProtocolLoader, get_protocols_list
from atlas.ot2.example_protocols.abstract_protocol import AbstractProtocol


class ProtocolManager:
    def __init__(
        self, protocol_name: str, protocol_parameters: Dict[str, np.ndarray]
    ):
    ...
    def spawn_protocol_file(self):
        ...
        s += (
            "protocol_parameters = "
            + json.dumps(self.protocol_parameters)
            + "\n\n"
        )
    ...
    def execute_protocol(self, simulation: Optional[bool] = False):
        ...
        if simulation:
            ...
            if True:
                Logger.log(
                    f'Simualtion of OT2 protocol "{self.protocol_name}" finished successfully',
                    "INFO",
                )
            else:
                Logger.log(
                    f'Simulation of OT2 protocol "{self.protocol_name}" failed!',
                    "FATAL",
                )
...
if __name__ == "__main__":
    ...
    manager = ProtocolManager(
        protocol_name="lnp_small_molecule", protocol_parameters=parameters
    )
