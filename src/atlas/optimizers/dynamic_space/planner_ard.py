...
class Dynamic_Flexible_Planner(BasePlanner):
    ...
    def _ask(self) -> List[ParameterVector]:
        ...
        if np.logical_or(
            ...
            Logger.log("Bounds {}".format(bounds), "INFO")
            ...
            if (regret + 1/n_iter**2 < 0):
                Logger.log('Regret < 0, redo the optimization', "INFO")
            ...
            if self.compute_expansion_trigger(regret, n_iter):
                Logger.log('Expanding bounds', "INFO")

                #train temp model using only expandable dims to expand the search space

                temp_reg_model = self.build_train_regression_gp(torch.squeeze(self.train_x_scaled_reg[:, self.expandable_dims]), self.train_y_scaled_reg, ard=True)
                kernel_k1 = temp_reg_model.covar_module.outputscale.detach().numpy().item()
                theta_n = np.sqrt(kernel_k1)
                # lengthscale = np.mean(temp_reg_model.covar_module.base_kernel.lengthscale.detach().numpy())

                # K = self.gram_matrix(self.train_x_scaled_reg[:, self.expandable_dims], kernel_k1, lengthscale)
                lengthscale = temp_reg_model.covar_module.base_kernel.lengthscale.detach().numpy().flatten()
                K = self.gram_matrix(torch.squeeze(self.train_x_scaled_reg[:, self.expandable_dims]), kernel_k1, np.mean(lengthscale))
                scale_l= self.compute_new_search_space(K, self.train_y_scaled_reg.numpy(), b_n, theta_n, kernel_k1)
                thetan_2 = kernel_k1
                radius = np.abs(np.max(bounds[:, 1]-bounds[:, 0]))

                b = np.mean(1/2*np.sqrt(2)*np.sqrt(thetan_2)/lengthscale)
                a = 1
                ...
                lengthscale_temp = np.zeros(len(self.func_param_space))
                i = 0
                for index in self.expandable_dims:
                    lengthscale_temp[index] = lengthscale[i]
                    i+=1
                
                Logger.log(f'lengthscale_temp: {lengthscale_temp}', "INFO")
                ...
                for n_obs in range(len(acq_init)):
                    ...
                    bounds_ori_temp = np.asarray((X0 - scale_l*lengthscale_temp,
                                                    X0 + scale_l*lengthscale_temp))
                    bounds_ori = bounds_ori_temp.T
                    bounds_ori_all.append(bounds_ori)

                self.bound_len = scale_l*lengthscale
                ...
                Logger.log('New bound: {}'.format(bounds_new), "INFO")
    ...
    def build_train_regression_gp(
        self, train_x: torch.Tensor, train_y: torch.Tensor, ard:bool=False
        ...
        if ard:
            rbfkernel = RBFKernel(ard_num_dims=train_x.size(dim=1)) 
        else:
            rbfkernel = RBFKernel()
    ...
    def argmax_infinity_reoptimize(
        ...
        Logger.log('Re-optimize within smaller spheres', "INFO")
