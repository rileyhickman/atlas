    ...
    def _ask(self) -> List[ParameterVector]:
        ...
        if np.logical_or(
            ...
            if self.compute_expansion_trigger(regret, n_iter):
                ...
                # for i in range(len(self.param_space.param_bounds)):
                #     print(i)
                #     temp_reg_model = self.build_train_regression_gp(self.train_x_scaled_reg[:,i].reshape(len(self.train_y_scaled_reg),1), self.train_y_scaled_reg)
                ...
                #     K = self.gram_matrix(self.train_x_scaled_reg[:,0].reshape(9,1).numpy(), kernel_k1, kernel_k2)


                #     scale_l= self.compute_new_search_space2(K, self.train_y_scaled_reg.numpy(), b_n, theta_n, kernel_k1)

                #     scale_ls.append(scale_l)
                # print(scale_l)


                # train gp using only expandable dims
