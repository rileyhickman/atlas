...
from .gps import ClassificationGP, CategoricalSingleTaskGP


from .acqfs import FeasibilityAwareEI, get_batch_initial_conditions, create_available_options
...
class BoTorchPlanner(CustomPlanner):
	...
	def build_train_regression_gp(self, train_x, train_y):
		...
		if self.problem_type == 'fully_continuous':
			...
			model = MixedSingleTaskGP(train_x, train_y, cat_dims=None)
		...
		if np.logical_or(
			...
			if self.problem_type == 'fully_continuous':
				...
				if self.feas_strategy == 'fca':
					# make an attempt to get some batch initial conditions
					# this may or may not be successful, depending on whether or not
					# there are avaialable feasible points as per the feasibility surrogate and threshold
					...
					if type(batch_initial_conditions) == type(None):
						...
						# if we have sufficient initial design points, use the constraint
						nonlinear_inequality_constraints = [self.fca_constraint]
					else:
						raise TypeError
			...
			elif self.problem_type=='fully_categorical' and self.has_descriptors:
