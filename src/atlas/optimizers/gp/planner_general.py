...
class BoTorchPlanner(CustomPlanner):
        ...
        **kwargs,
        ...
        if self.is_moo:
            if tolerances is None:
                print(
                    f"[FATAL]: You must define Chimera tolerances for multiobjective optimization"
                )
                quit()
            if absolutes is None:
                print(
                    f"[FATAL]: You must define absolutes for multiobjective optimization"
                )
                quit()
            if goals is None:
                print(
                    f"[FATAL]: You must define goals for each objective in multiobjective optimization"
                )
                quit()

            if self.goal == "maximize":
                print(
                    "f [WARNING]: Overall goal must be set to minimization for multiobjective optimization. Updating ..."
                )
                self.goal = "minimize"
    ...
    def train_vgp(self, model, likelihood, train_x, train_y):
        ...
        print(
            f" >>> [{self.vgp_iters} epochs] VGP trained in {round(vgp_train_time,3)} sec \t Loss : {round(loss.item(), 3)} "
        )
    ...
    def build_train_data(self):
        ...
        if self.is_moo:
            ...
            train_y_reg = self.chimera.scalarize(train_y_reg).reshape(
                -1, 1
    ...
    def _tell(self, observations):
        ...
        if type(observations) == list:
            ...
            if len(observations) != len(self.tolerances):
                print(
                    f"[FATAL]: number of objectives (len(observations)) does not equal number of tolerances (len(self.tolerances))"
                )
                quit()
            ...
            self._values = np.stack(
                self._values, axis=-1
            )  # (num_observations, num_objectives)
        ...
        if np.logical_or(
            ...
            if self.problem_type == "fully_continuous":
                ...
                if callable(self.known_constraints):
                    ...
                    nonlinear_inequality_constraints.append(
                ...
                if self.feas_strategy == "fca":
                    ...
                    if type(batch_initial_conditions) == type(None):
                        ...
                        if type(batch_initial_conditions) == type(None):
                            ...
                            print(
                                "Could not find inital conditions for constrianed optimization..."
                            )
                            quit()
                    ...
                    if callable(self.known_constraints):
                        ...
                        if type(batch_initial_conditions) == type(None):
                            ...
                            print(
                                "[FATAL] Could not find inital conditions for constrianed optimization..."
                            )
                            quit()

                if (
                    not self.known_constraints
                    and not self.feas_strategy == "fca"
                ):
                ...
                cat_feat = torch.tensor(
                    cat_param_to_feat(
                        self.param_space[0],
                        self.param_space[0].options[select_ix],
                    )
                )
            ...
            if (
                self.problem_type in ["fully_categorical", "mixed"]
                and not self.has_descriptors
            ):
...
if __name__ == "__main__":
    ...
    if PARAM_TYPE == "continuous_nan":
        ...
        for num_iter in range(BUDGET):
            ...
            print(
                f"ITER : {num_iter}\tSAMPLES : {samples}\t MEASUREMENT : {measurement}"
            )
            ...
            if PLOT and num_iter > 4:
                ...
                domain_norm = forward_normalize(
                    domain, planner._mins_x, planner._maxs_x
                )

                torch_domain_reg = torch.tensor(
                    domain_norm.reshape(-1, 1)
                ).double()
                torch_domain_cla = torch.tensor(domain_norm).float()
                torch_domain_acqf = (
                    torch.tensor(domain_norm)
                    .float()
                    .view(domain_norm.shape[0], 1, 1)
                )

                surf_vals = np.array(
                    [constrained_surface(np.array([x])) for x in domain]
                )
                ...
                for obs_ix, (param, val) in enumerate(zip(params, values)):
                    if np.isnan(val):
                        axes[0].plot(
                            param[0], -1, marker="x", color="k", markersize=10
                        )
                    else:
                        axes[0].plot(
                            param[0], val, marker="o", color="k", markersize=10
                        )
                        axes[0].plot(
                            param[0],
                            val,
                ...
                reg_mu = reverse_standardize(
                    reg_mu, planner._means_y, planner._stds_y
                )
                ...
                p_feas = (
                    planner.cla_likelihood(planner.cla_model(torch_domain_cla))
                    .mean.detach()
                    .numpy()
                )
        ...
        for iter in range(BUDGET):
            ...
            print(
                f'ITER : {iter}\tSAMPLES : {samples}\tMEASUREMENT : {measurement["obj"]}'
            )
        ...
        for num_iter in range(BUDGET):
            ...
            samples = planner.recommend(
                [campaign1.observations, campaign2.observations]
            )
            for sample in samples:
                sample_arr = sample.to_array()
            ...
            print(
                f"ITER : {num_iter}\tSAMPLES : {samples}\t MEASUREMENT 1 : {measurement1}\t MEASUREMENT 2: {measurement2}"
            )
        ...
        from benchmark_functions import CatDejongConstr, CatMichalewiczConstr
        ...
        for iter in range(BUDGET):
            ...
            campaign1.add_observation(
                samples[0].to_array(), measurement1["obj"]
            )
            campaign2.add_observation(
                samples[0].to_array(), -measurement2["obj"]
            )
            ...
            print(
                f'ITER : {iter}\tSAMPLES : {samples}\tMEASUREMENT : {measurement1["obj"]}\tMEASUREMENT : {measurement2["obj"]}'
            )
        ...
        for i in range(NUM_RUNS):
            ...
            for iter in range(BUDGET):
                ...
                print(measurement)
                print(
                    f"ITER : {iter}\tSAMPLES : {samples}\t MEASUREMENT : {measurement[0][0]}"
                )
        ...
        lookup_df = pickle.load(
            open("datasets_emulators/perovskites/perovskites.pkl", "rb")
        )
        ...
        pickle.dump(
            all_campaigns, open("results/perovskites_botorch_naive.pkl", "wb")
        )
        ...
        pickle.dump(
            all_campaigns, open("results/perovskites_random.pkl", "wb")
        )
        ...
        def get_descriptors(element, kind):
            ...
            return (
                lookup_df.loc[(lookup_df[kind] == element)]
                .loc[:, lookup_df.columns.str.startswith(f"{kind}-")]
                .values[0]
                .tolist()
            )
        ...
        for i in range(NUM_RUNS):
            ...
            organic_param = ParameterCategorical(
                ...
                options=organic_options,
                descriptors=[
                    get_descriptors(option, "organic")
            ...
            anion_param = ParameterCategorical(
                ...
                options=anion_options,
                descriptors=[
                    get_descriptors(option, "anion")
                    for option in anion_options
                ],
            )
            param_space.add(anion_param)
            ...
            cation_param = ParameterCategorical(
                ...
                options=cation_options,
                descriptors=[
                    get_descriptors(option, "cation")
                    for option in cation_options
                ],
            )
            param_space.add(cation_param)
        ...
        pickle.dump(
            all_campaigns,
            open("results/perovskites_botorch_descriptors.pkl", "wb"),
        )
