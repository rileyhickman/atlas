...
class RGPEPlanner(BasePlanner):
		...
		for task_ix, task in enumerate(self._train_tasks):
			...
			source_models.append(
				self._get_fitted_model(
					torch.tensor(task['params']), torch.tensor(task['values']),
				)
			)
	...
	def get_target_model_loocv_sample_preds(self, train_x, train_y, target_model, num_samples):
		"""
		...
		masks = torch.eye(len(train_x), dtype=torch.uint8, device=self.device).bool()
		...
		model = self._get_fitted_model(train_x_cv, train_y_cv, state_dict=state_dict_expanded)
		with torch.no_grad():
	...
	def _ask(self):
		...
		if np.logical_or(
			...
			if self.cache_weights:
				...
				self.all_ranking_losses.append(ranking_loss_tensor.detach().numpy())
				os.makedirs(self.weights_path, exist_ok=True)
				with open(os.path.join(self.weights_path, 'rank_weights.pkl'), 'wb') as f:
					pickle.dump({'weights': self.all_rank_weights, 'losses': self.all_ranking_losses}, f)
